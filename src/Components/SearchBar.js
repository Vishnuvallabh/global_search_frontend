...
// function SearchBar({ setSearchTime, setSearchResults, setTotalResults, setPage }) {
//   const [columns, setColumns] = useState([]);
//   const [numericColumns, setNumericColumns] = useState([]); // New state for numeric columns
//   const [conditions, setConditions] = useState([{ column: '', query: '', operator: '' }]);
//   const [loading, setLoading] = useState(false);
//   const [queryHistory, setQueryHistory] = useState(() => {
//     const storedQueries = localStorage.getItem('queryHistory');
...
//       .then(response => response.json())
//       .then(data => {
//         setColumns(data.GlobalSearchData.all_columns || []);
//         setNumericColumns(data.GlobalSearchData.numeric_columns || []); // Set numeric columns
//       })
//       .catch(error => console.error('Error fetching columns:', error));
//   }, []);
...
//             {/* New Numeric Columns Dropdown */}
//             <select
//               value={condition.column}
//               onChange={(e) => handleConditionChange(index, 'column', e.target.value)}
//               style={{ flex: 1 }}
//             >
//               <option value="">Numeric Columns</option>
//               {numericColumns.map((numericColumn, idx) => (
//                 <option key={idx} value={numericColumn}>
//                   {numericColumn}
//                 </option>
//               ))}
//             </select>
...
function SearchBar({ setSearchTime, setSearchResults, setTotalResults, setPage }) {
  ...
  const fetchResults = useCallback(async (page = 1) => {
    ...
    const readableQueryString = conditions
      .filter(condition => condition.query)
      .map(condition => {
        if (condition.column && numericColumns.includes(condition.column)) {
          // For numeric columns, omit the colon in the readable query string
          return `${condition.column}${condition.query}`;
    ...
    const apiQueryString = conditions
      ...
      .map(condition => {
        if (condition.column && numericColumns.includes(condition.column)) {
          // For numeric columns, omit the colon in the API query string
          return `${encodeURIComponent(condition.column)}${encodeURIComponent(condition.query)}`;
        } else if (condition.column) {
          return `${encodeURIComponent(condition.column)}%3A${encodeURIComponent(condition.query)}`;
        } else {
          return `${encodeURIComponent(condition.query)}`;
        }
      })
  ...
  }, [conditions, queryHistory, setPage, setSearchResults, setSearchTime, setTotalResults, numericColumns]);
